    ...
    for i, lead in enumerate(st.session_state.scored_leads):
        ...
        with lead_container:
            ...
            with content_col:
                ...
                with st.expander(f"Score: {lead['score']}/100 | Confidence: {lead.get('confidence', 50)}/100 - {lead['timestamp']}", expanded=False):
                    # Lead description
...
with st.sidebar:
    ...
    *Border colors use the same gradient as scores*
    ...
    if st.session_state.scored_leads:
        ...
        confidences = [lead.get("confidence", 50) for lead in st.session_state.scored_leads]
