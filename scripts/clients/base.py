    ...
    def load_client_config(self, client_type: str) -> Dict[str, Any]:
        ...
        for section_name, section_configs in configs.items():
            if client_type in section_configs:
                ...
                client_config['_section'] = section_name  # Add metadata about which section it came from
                self.logger.debug(f"Loaded client config for '{client_type}' from section '{section_name}'")
