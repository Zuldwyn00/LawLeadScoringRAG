    ...
    def score_lead_process(self, lead_description: str, progress_callback=None, completion_callback=None, error_callback=None):
        ...
        try:
            ...
            if progress_callback:
                progress_callback(10, "🔧 Initializing AI clients and managers...", time.time() - start_time)
            qdrant_manager, lead_scoring_client, embedding_client = self.initialize_managers()
            
            # Step 2: Generate embeddings
            if progress_callback:
                progress_callback(25, "🧠 Generating embeddings for lead description...", time.time() - start_time)
            question_vector = embedding_client.get_embeddings(lead_description)
            
            # Step 3: Search for similar cases
            if progress_callback:
                progress_callback(45, "🔍 Searching for similar historical cases...", time.time() - start_time)
            ...
            if progress_callback:
                progress_callback(60, "📚 Retrieving historical context...", time.time() - start_time)
            historical_context = qdrant_manager.get_context(search_results)
            
            # Step 5: AI analysis with animation
            if progress_callback:
                progress_callback(65, "⚖️ Analyzing lead with AI scoring system...", time.time() - start_time)
            ...
            if progress_callback:
                progress_callback(95, "📊 Extracting score and confidence metrics...", time.time() - start_time)
    ...
    def _animate_ai_progress(self, start_time, progress_callback):
        ...
        while self.ai_analysis_running:
            try:
                ...
                if progress_callback:
                    progress_callback(new_progress, status_text, time.time() - start_time)
...
class UIEventHandler:
    ...
    def handle_score_lead_clicked(self, lead_text: str):
        ...
        if not lead_text.strip() or lead_text.strip() == "Enter the detailed description of the potential case...":
