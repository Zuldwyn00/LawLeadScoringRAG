...
def get_chat_logs_directory() -> Path:
    ...
    """
    config = load_config()
    chat_logs_dir = config.get('directories', {}).get('chat_logs', 'scripts/data/chat_logs')
...
def load_all_scored_leads(chat_logs_directory: str | Path | None = None) -> List[ScoredLead]:
    """
...
def filter_scored_leads(
    ...
    if min_score is not None:
        filtered_leads = [lead for lead in filtered_leads if lead.lead_score >= min_score]
    
    if max_score is not None:
        filtered_leads = [lead for lead in filtered_leads if lead.lead_score <= max_score]
    
    if days_back is not None:
        cutoff_date = datetime.now() - timedelta(days=days_back)
        filtered_leads = [lead for lead in filtered_leads if lead.timestamp >= cutoff_date]
